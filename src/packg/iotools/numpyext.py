...
def dumps_numpy_array(arr) -> Tuple[bytes, str]:
    ...
    Returns: tuple of
        ...
        str: dtype and shape as string e.g. "float32-100,100" for a 100x100 float32 array
             or "float64-" for a scalar (zero-dimensional array)

    """
    # Handle zero-dimensional arrays (scalars)
    if arr.ndim == 0:
        return arr.tobytes(), f"{arr.dtype.name}-"
...
def loads_numpy_array(arr, dtype_shape) -> np.ndarray:
    ...
    # Handle zero-dimensional arrays (scalars)
    if not shape:
        return np.frombuffer(arr, dtype=dtype).reshape(())
